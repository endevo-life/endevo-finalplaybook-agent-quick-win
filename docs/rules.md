...
Source of truth is `agent/rules_engine.py` + `knowledge-base/niki-content-library.json`.
This file is a readable summary for review — if it drifts from the code, the
code wins; fix this doc to match.

**Status: DRAFT.** This logic is extracted from Niki's real client sessions
but has not yet been formally validated by Niki or Andrea (see
`niki-content-library.json` → `_meta.status`). Treat the priority order below
...
|---|---|---|---|
| 1 | `hasAgingParent AND parentHasNoDocs` | `profile_aging_parent` | Highest urgency in Niki's sessions — "I just don't want you to get the call and not be able to act." |
...
| 5 | `hasSomeDone OR hasWill` | `profile_pressure_test` | Existing documents ≠ a working plan — Niki's rule is "pressure test," not "assume done." |
...
`isBusinessOwner` is true. Niki is explicit that personal and business
...
(`MAX_ACTION_ITEMS_PER_PLAN` in `rules_engine.py`), matching Niki's stated
